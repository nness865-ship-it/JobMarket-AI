...
def _get_skill_categories():
    """Define skill categories for automatic field detection."""
    return {
        "computer_science": [
...
def _detect_skill_category(skills):
    """Detect the primary category based on user skills."""
    if not skills:
        return "general"
    ...
    if category_scores:
        ...
        # Only return category if confidence is above 20%
        if best_score >= 20:
...
def _normalize_skills(skills):
    """Lowercase, strip, deduplicate and standardize skills."""
    standard_map = {
...
def _skill_vocabulary():
    """Build a comprehensive vocabulary from static list + skills already in jobs collection."""
    static = [
        # Programming & Tech
        ...
        "pnpm", "ruby", "rails", "php", "laravel", "symfony", "c++", "c#", "dot net",
...
def _extract_skills_from_text(text: str):
    ...
    # Common tech patterns (e.g., C++, .NET, Node.js)
    tech_patterns = [
    ...
    # 3. Vocabulary Matcher (Word Boundaries)
    for term in vocab:
        if re.search(rf'\b{re.escape(term)}\b', text_lower):
            hits.add(term)

    # 4. Universal Heuristic: Find potential skills (Capitalized words/phrases)
    # This captures "new" technologies not in our static list
    lines = text.split('\n')
    ...
    for line in lines:
        ...
        if in_skill_section:
            # Look for CamelCase or Title Case words separated by commas or bullets
            # e.g. "React Native, SpringBoot, Kubernetes"
    ...
    # Optional: Use Gemini for advanced extraction if key is available
    if GOOGLE_API_KEY:
        try:
...
def upload_resume():
    ...
    if not email:
        # Use a generic guest ID if no email is provided to allow temporary processing
    ...
    # Detect category based on extracted skills
...
def save_user_skills():
    ...
    # Detect category based on skills
...
def update_user_profile():
    """Update user's professional profile with domain-specific information."""
    data = request.json
    email = _current_email() or data.get("email")
...
def recommend_jobs():
    ...
    # If no user found or no email, use skills from request or empty
    ...
    for job in all_jobs:
        ...
        # Boost score if job is in same category as user
        category_boost = 0
        is_category_match = job_category == user_category and user_category != "general"
        if is_category_match:
            category_boost = 15  # 15% boost for same category
        
        # Additional boost for high skill overlap
        overlap_boost = min(10, skill_overlap * 0.2)  # Up to 10% boost for skill overlap
    ...
    # Sort category jobs by match score
    category_jobs.sort(key=lambda x: x["match"], reverse=True)
    other_jobs.sort(key=lambda x: x["match"], reverse=True)
    
    # Combine recommendations: prioritize category matches, then best others
    recommendations = category_jobs[:8] + other_jobs[:4]  # Max 12 recommendations
    
    # Generate skill suggestions for career growth
...
def _generate_skill_suggestions(user_skills, user_category, missing_skills):
    """Generate skill suggestions for career growth based on user's field."""
    
    # Field-specific skill growth paths
    growth_paths = {
        "computer_science": {
    ...
    # Get suggestions for user's category
    category_suggestions = growth_paths.get(user_category, {})
    
    suggestions = []
    user_skills_set = set(user_skills)
    
    # Add category-specific growth paths
    for path_name, path_skills in category_suggestions.items():
        ...
        if missing_in_path:
            suggestions.append({
                ...
                "skills": missing_in_path[:3],  # Top 3 skills
                "priority": "high" if len(missing_in_path) <= 2 else "medium"
            })
    
    # Add popular missing skills from job market
    popular_missing = list(missing_skills)[:10]  # Top 10 most requested missing skills
    if popular_missing:
        suggestions.append({
...
def save_skill_progress():
    ...
    update_query = {}
    if is_completed:
        # Add to completed steps and tracker data
        update_query = {
...
@app.route("/seed-jobs")
def seed_jobs():
    """Seed database with a comprehensive set of job roles across all fields."""
    sample_jobs = [
        # Technology & Programming - More variety
        {
            "role": "data analyst",
        ...
        # Engineering & Sciences - More specializations
        {
        ...
        # Medical & Healthcare - More specializations
        {
        ...
        # Business & Commerce - More variety
        {
        ...
        # Science & Research - More specializations
        {
        ...
        # Agriculture & Environmental - More roles
        {
        ...
        # Psychology & Social Sciences - More roles
        {
...
def auth_demo():
    """Demo login - creates a temporary demo user account."""
    demo_email = f"demo.user.{random.randint(1000, 9999)}@elevateai.demo"
    demo_name = "Demo User"
    
    # Pre-populate with biochemical engineering skills for demo
    demo_skills = [
...
def get_promotion_pathway():
    """Generate domain-driven promotion pathway based on user's professional profile."""
    data = request.json
    email = _current_email() or data.get("email")
    skills = data.get("skills", [])
    
    # Get profile data from request or database
    current_job_role = (data.get("current_job_role") or "").strip()
    job_domain = (data.get("job_domain") or "").strip()
    position_level = (data.get("position_level") or "").strip()
    current_salary = data.get("current_salary", 0)
    
    # If not provided in request, fetch from user profile
    if not current_job_role or not job_domain or not position_level:
        if email:
            user = users_collection.find_one({"email": email}, {"_id": 0})
            if user:
    ...
    # Build comprehensive prompt with all profile context
    prompt = f"""
    You are a Senior Career Strategy Architect specializing in {job_domain}.
    
    USER PROFESSIONAL PROFILE:
    - Current Role: {current_job_role}
    - Domain: {job_domain}
    - Position Level: {position_level}
    - Current Salary: ${current_salary:,} (if applicable)
    - Skills: {', '.join(normalized_skills) if normalized_skills else 'Not specified'}
    
    CRITICAL INSTRUCTIONS:
    1. Generate ONLY {job_domain}-specific career guidance. Do NOT suggest roles outside this domain.
    2. Tailor promotion pathway to {job_domain} industry standards and career progression.
    3. Projects MUST be realistic for {job_domain} professionals:
       - Marketing: Campaign execution, ROI analysis, market research
    ...
    4. Salary progression should reflect {job_domain} industry benchmarks.
    5. Skills should be domain-specific tools and competencies.
    6. Leadership skills should align with {job_domain} management practices.
    
    Generate a precise, realistic, and actionable promotion pathway for a {position_level}-level {current_job_role} in {job_domain}.
    ...
    {{
      ...
      "domain": "{job_domain}",
      "level": "{position_level}",
      "next_role": "String (logical next promotion level in {job_domain})",
      "salary_progression": {{
        "current": {current_salary},
        "next_level": "Estimated salary at next level",
        "senior_level": "Estimated salary at senior level"
      }},
      "skills_to_work_on": {{
        "core": ["Domain-specific skill 1", "Domain-specific skill 2", "Domain-specific skill 3"],
        "tools": ["Tool/Platform 1 for {job_domain}", "Tool/Platform 2 for {job_domain}"],
        "leadership": ["Leadership skill 1", "Leadership skill 2"],
        "impact": ["Business impact skill 1", "Business impact skill 2"]
      }},
      "projects": [
        {{
          "project": "Project Name",
          "objective": "Real-world objective for {job_domain}",
          "skills_gained": ["Skill 1", "Skill 2"],
          "promotion_impact": "How it proves readiness for promotion"
        }}
      ],
      "strategy": {{
        "apply_in_job": "How to apply these skills in current {job_domain} role",
        "show_results": "How to demonstrate measurable impact in {job_domain}",
        "position_for_promotion": "How to position for promotion in {job_domain}"
      }}
    }}
    """
...
def get_high_paying_jobs():
    """Find high-paying jobs strictly aligned with user's domain and role."""
    data = request.json
    email = _current_email() or data.get("email")
    ...
    if not job_domain:
        # Fallback to skill-based domain detection
    ...
    for job in domain_filtered_jobs:
        ...
        if "$" in salary_str or "k" in salary_str:
            ...
            # Simulated estimate for demo purposes
        ...
        # Rule: Salary threshold (higher than $90k for "high-paying")
    ...
    # 3. DOMAIN-SPECIFIC MATCH CALCULATION
    results = []
    ...
    for job in filtered_jobs:
        ...
        # Weights: Core 50%, Experience/Domain 30%, Tools 20%
        ...
        # Experience proxy: based on skill count and role relevance
        ...
        if current_job_role and any(word in job.get("role", "").lower() for word in current_job_role.lower().split()):
            ...
            # Boost if position level matches
        ...
        # Domain-specific qualification steps
        qualify_steps = []
        if missing_skills:
...
def get_job_trends():
    """Returns comprehensive market intelligence and trends."""
    try:
        all_jobs = list(jobs_collection.find({}, {"_id": 0}))
    except Exception:
        all_jobs = []
    ...
    # Real-time comparisons for "Interactive" feel
